  ...
  return {
    ...
    service: {
      ...
      transform(input, options2, isTTY2, callback) {
        ...
        sendRequest(["transform", {flags, input: input + ""}], (error, response) => {
          if (error)
            return callback(new Error(error), null);
          let errors = response.errors;
          let warnings = response.warnings;
...
let startService = (options) => {
  ...
  return Promise.resolve({
    ...
    transform: (input, options2) => new Promise((resolve, reject) => service.transform(input, options2 || {}, isTTY(), (err, res) => err ? reject(err) : resolve(res))),
    stop() {
      child.kill();
    }
  });
